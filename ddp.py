...
def ddp_loader(train_dataset, batch_size, rank, world_size):
    pass
...
def self_KD_Dropout_train_epoch(train_loader, teacher, student, optimizer, device, scaler, lr_scheduler, temperature=3, alpha=0.01, beta=1e-6, hidden_rep_loss_weight=0.15, hlambda=0.1, use_amp=True):
    logging.log(logging.INFO, 'Training self KD Dropout with temperature = {} and alpha = {} and beta = {} and hidden_rep_loss_weight = {} and hlambda = {}'.format(
        temperature, alpha, beta, hidden_rep_loss_weight, hlambda))
    running_loss = 0
    ...
    for batch_x, batch_y in train_loader:
        ...
        with torch.autocast(device_type=device, dtype=torch.float16, enabled=use_amp):
            ...
            batch_out, batch_out2, spectral_output, temporal_output, graph_output_S, graph_output_T, hs_gal_output_S, hs_gal_output_T, middle_feature1, middle_feature2, final_feature1, final_feature2, student_hidden_representation = student(
                batch_x)
            ...
            loss_kl_1 = nn.functional.kl_div(torch.nn.functional.log_softmax(
                batch_out, dim=1), torch.nn.functional.softmax(batch_out2, dim=1), reduction="batchmean")

            loss_kl_2 = nn.functional.kl_div(torch.nn.functional.log_softmax(
                batch_out2, dim=1), torch.nn.functional.softmax(batch_out, dim=1), reduction="batchmean")

            total_kl_loss = loss_kl_1 + loss_kl_2

            # Now pass the reshaped tensors to cosine_loss
            hidden_rep_loss = cosine_loss(
                student_hidden_representation, teacher_hidden_representation, target=torch.ones(batch_size).to(device))
            ...
            # Total loss
            total_loss = (1 - alpha) * total_label_loss + alpha * total_kd_loss + beta * \
                total_feature_loss + hidden_rep_loss_weight * \
                hidden_rep_loss + hlambda * total_kl_loss
...
def self_KD_train_epoch(train_loader, model, optimizer, device, scaler, temperature=3, alpha=0.1, beta=1e-6, use_amp=True):
    logging.log(logging.INFO, 'Training self KD with temperature = {} and alpha = {} and beta = {}'.format(
    ...
    for batch_x, batch_y in train_loader:
        ...
        with torch.autocast(device_type=device, dtype=torch.float16, enabled=use_amp):
            ...
            total_loss = (1 - alpha) * total_label_loss + alpha * \
                total_kd_loss + beta * total_feature_loss
...
def self_KD_teacher_train_epoch(train_loader, student, teacher, optimizer, device, scaler, temperature=3, alpha=0.1, beta=1e-6, hidden_rep_loss_weight=0.5, use_amp=True):
    logging.log(logging.INFO, 'Training self KD + teacher cosine with temperature = {} and alpha = {} and beta = {} and hidden_rep_loss_weight = {}'.format(
        temperature, alpha, beta, hidden_rep_loss_weight))
    running_loss = 0
    running_total_label_loss = 0
    running_total_kd_loss = 0
    running_total_feature_loss = 0
    ...
    for batch_x, batch_y in train_loader:
        ...
        with torch.autocast(device_type=device, dtype=torch.float16, enabled=use_amp):
            ...
            total_loss = (1 - alpha) * total_label_loss + alpha * total_kd_loss + \
                beta * total_feature_loss + hidden_rep_loss_weight * hidden_rep_loss
...
def self_KD_teacher2_train_epoch(train_loader, student, teacher, optimizer, device, scaler, temperature=3, alpha=0.1, beta=1e-6, hidden_rep_loss_weight=0.5, use_amp=True):
    logging.log(logging.INFO, 'Training self KD + teacher 2 cosine with temperature = {} and alpha = {} and beta = {} and hidden_rep_loss_weight = {}'.format(
        temperature, alpha, beta, hidden_rep_loss_weight))
    running_loss = 0
    running_total_label_loss = 0
    running_total_kd_loss = 0
    ...
    for batch_x, batch_y in train_loader:
        ...
        with torch.autocast(device_type=device, dtype=torch.float16, enabled=use_amp):
            ...
            # Calculate total loss
            # Total label loss
            total_label_loss = batch_loss + spectral_loss + \
                temporal_loss + hs_gal_loss_S + hs_gal_loss_T

            # Total KD loss
            total_kd_loss = kd_spectral_loss + kd_temporal_loss + \
                kd_hs_gal_loss_S + kd_hs_gal_loss_T
    ...
    with torch.inference_mode():
        for batch_x, batch_y in dev_loader:
            ...
            if kd_method == 'self_KD_Teacher':
                ...
                batch_out, batch_out2, spectral_output, temporal_output, graph_output_S, graph_output_T, hs_gal_output_S, hs_gal_output_T, middle_feature1, middle_feature2, final_feature1, final_feature2, hidden_features = model(
                    batch_x)
            batch_y = batch_y.view(-1).type(torch.int64).to(device)
...
def produce_evaluation_file(dataset, model, device, save_path, kd_method=None, batch_size=4):
    data_loader = DataLoader(dataset, batch_size=batch_size, shuffle=False,
                             drop_last=False, pin_memory=True, pin_memory_device=device)
    model.eval()
    fname_list = []
    score_list = []
...
def get_train_dev_dataloader(args):
    ...
    train_loader = DataLoader(train_set, batch_size=args.batch_size, num_workers=2,
                              shuffle=True, drop_last=True, pin_memory=True, pin_memory_device=device)
...
if __name__ == '__main__':
    ...
    if not args.self_KD or args.self_KD_type == 'self_KD_Teacher_HG' or args.self_KD_type == 'self_KD_Teacher_HG_Cosine_Dropout' or \
            args.self_KD_type == 'self_KD_Teacher_SSL_WAV2VEC2_BASE_TA' or args.self_KD_type == 'self_KD_Teacher_SSL_WAV2VEC2_ASR_BASE_960H_TA':
    ...
    if args.eval:
        ...
        produce_evaluation_file(eval_set, model if args.is_eval_teacher else student,
                                device, args.eval_output, batch_size=args.batch_size_eval, kd_method=kd_method)
        sys.exit(0)
    ...
    for epoch in range(start_epoch, num_epochs):
        if args.KD_logits:
            ...
            if args.self_KD_type == 'self_KD_Teacher_HG' or args.self_KD_type == 'self_KD_Teacher_SSL_WAV2VEC2_BASE_TA' or args.self_KD_type == 'self_KD_Teacher_SSL_WAV2VEC2_ASR_BASE_960H_TA':
                ...
                running_loss, running_total_label_loss, running_total_kd_loss, running_total_feature_loss = self_KD_train_epoch(
                    train_loader, student, optimizer, device, scaler, use_amp=args.use_amp)
