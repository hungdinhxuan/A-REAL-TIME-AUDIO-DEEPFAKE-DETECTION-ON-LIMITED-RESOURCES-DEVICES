...
def get_train_dev_dataloader(args):
    ...
    dev_loader = DataLoader(dev_set, batch_size=args.batch_size,num_workers=2, shuffle=False, pin_memory=True, pin_memory_device=device)
...
if __name__ == '__main__':
    ...
    for epoch in range(start_epoch, num_epochs):
        if args.KD_logits:
            ...
            if args.ssl_type != 'Distil_XLSR':
                running_loss = train_kd_mse_loss(model, student, train_loader, optimizer, feature_map_weight=0.742208, ce_loss_weight=0.483691, device=device)
            else:
