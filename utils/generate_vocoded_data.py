...
m_hifigan_vocoder = hifigan.ModelGenerator(
    model_hifigan_config.input_dim, model_hifigan_config.output_dim, None, model_hifigan_config)
...
hn_sinc_nsf_vocoder = hn_sinc_nsf.Model(
    hn_sinc_nsf_config.input_dim, hn_sinc_nsf_config.output_dim, None, hn_sinc_nsf_config)
...
hn_sinc_nsf_hifi_vocoder = hn_sinc_nsf_hifi.ModelGenerator(
    hn_sinc_nsf_hifi_config.input_dim, hn_sinc_nsf_hifi_config.output_dim, None, hn_sinc_nsf_hifi_config)
...
m_waveglow_vocoder = waveglow.Model(
    waveglow_config.input_dim, waveglow_config.output_dim, None, waveglow_config)
...
for root, dirs, files in os.walk(VOXCELEB2_DATASET_PATH):
    for file in files:
        if file.endswith('.wav'):
            ...
            wav_hifigan_save_path = os.path.join(VOCODED_DATASET_PATH, root, file.replace(
                '.wav', '_hifigan.wav')).replace('voxceleb2', 'voxceleb2_vocoded')
            wav_hn_sinc_nsf_save_path = os.path.join(VOCODED_DATASET_PATH, root, file.replace(
                '.wav', '_hn_sinc_nsf.wav')).replace('voxceleb2', 'voxceleb2_vocoded')
            wav_hn_sinc_nsf_hifi_save_path = os.path.join(VOCODED_DATASET_PATH, root, file.replace(
                '.wav', '_hn_sinc_nsf_hifi.wav')).replace('voxceleb2', 'voxceleb2_vocoded')
            wav_waveglow_save_path = os.path.join(VOCODED_DATASET_PATH, root, file.replace(
                '.wav', '_waveglow.wav')).replace('voxceleb2', 'voxceleb2_vocoded')
            ...
            with torch.no_grad():
                ...
                # HIFI-GAN
                if not os.path.exists(wav_hifigan_save_path):
                    input_feat_hifigan = torch.tensor(
                        mel, dtype=torch.float32, device=device).unsqueeze(0)
                    wav_hifigan = m_hifigan_vocoder(input_feat_hifigan)
                    wav_hifigan = wav_hifigan[0, :, 0].cpu().numpy()
                    wav_tools.waveFloatToPCMFile(
                        wav_hifigan, wav_hifigan_save_path)
                # HN-SINC-NSF

                if not os.path.exists(wav_hn_sinc_nsf_save_path):
                    input_feat_wav_hn_sinc_nsf = torch.tensor(np.concatenate([mel, np.expand_dims(f0, axis=1)], axis=1),
                                                              dtype=torch.float32, device=device).unsqueeze(0)
                    wav_hn_sinc_nsf = hn_sinc_nsf_vocoder(
                        input_feat_wav_hn_sinc_nsf)
                    wav_hn_sinc_nsf = wav_hn_sinc_nsf[0].cpu().numpy()
                    wav_tools.waveFloatToPCMFile(
                        wav_hn_sinc_nsf, wav_hn_sinc_nsf_save_path)

                # HN-SINC-NSF-HIFI
                if not os.path.exists(wav_hn_sinc_nsf_hifi_save_path):

                    input_feat_wav_hn_sinc_nsf_hifi = torch.tensor(np.concatenate([mel, np.expand_dims(f0, axis=1)], axis=1),
                                                                   dtype=torch.float32, device=device).unsqueeze(0)
                    wav_hn_sinc_nsf_hifi = hn_sinc_nsf_hifi_vocoder(
                        input_feat_wav_hn_sinc_nsf_hifi)
                    wav_hn_sinc_nsf_hifi = wav_hn_sinc_nsf_hifi[0, :, 0].cpu(
                    ).numpy()
                    wav_tools.waveFloatToPCMFile(
                        wav_hn_sinc_nsf_hifi, wav_hn_sinc_nsf_hifi_save_path)

                # WAVEGLOW
                if not os.path.exists(wav_waveglow_save_path):
                    input_feat_waveglow = torch.tensor(
                        mel, dtype=torch.float32, device=device_cpu).unsqueeze(0)

                    wav_waveglow = m_waveglow_vocoder.inference(
                        input_feat_waveglow)

                    wav_waveglow = wav_waveglow[0, :, 0].numpy()

                    wav_tools.waveFloatToPCMFile(
                        wav_waveglow, wav_waveglow_save_path)
