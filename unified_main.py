...
wandb_project_name = config["train"].get("wandb_project_name", "torchdistill_unified")
...
{
    'params': unified_loss.parameters(),
}
], lr=float(
...
for epoch in tqdm(range(start_epoch, num_epochs), colour='green'):
    ...
    if "self_kd_config" not in config:
        train_loss, train_acc, loss_dict = kd_train_epoch_unified(train_loader, student_model, teacher_model, optimizer, device, scaler, config,
                                                              student_forward_hook_manager, teacher_forward_hook_manager, epoch, exp_lr_scheduler=exp_lr_scheduler, use_amp=use_amp, unified_loss=unified_loss)
       
        # dev_loss, accuracy = kd_val_epoch(
        #     dev_loader, student_model, device, config)
        # new eval
        dev_loss, accuracy, dev_loss_dict = kd_val_epoch_unified(
            dev_loader, student_model, teacher_model, device, config, student_forward_hook_manager, teacher_forward_hook_manager, unified_loss=unified_loss)
