    ...
    for i, (batch_x, batch_y) in pbar:
        ...
        with torch.autocast(device_type=device, dtype=torch.float16, enabled=use_amp):
            ...
            if standard_mid_loss is not None:
                ...
                #print(f"Length student feat extracted: {len(student.module.front_end.extract_layer_results(batch_x))}")
                tmp_loss, tmp_mse_loss, tmp_cosine_loss = standard_mid_loss.forward(student_io_dict, teacher_io_dict, student_module_path_list, student_module_io_list, teacher_module_path_list, teacher_module_io_list, size=batch_size)
