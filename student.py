...
class Custom_Wav2Vec2_Fe_AASIST(nn.Module):
...
@register_model(key='Distil_Wav2vec2_Custom_ConformerTCM')
class Distil_Wav2vec2_Custom_ConformerTCM(nn.Module):
    ...
    def __init__(self, device, args, **kwargs):
        ...
        # create network wav2vec 2.0
        ####
        self.front_end = Custom_Wav2Vec2_Fe(device, 
            out_dim=256,
            ffn_dim=1536,
        ).to(device)
        self.LL = nn.Linear(256, args['emb_size'])
        self.first_bn = nn.BatchNorm2d(num_features=1)
        self.selu = nn.SELU(inplace=True)
        self.backend=MyConformer(**args)
