...
class MultiHeadAttention(nn.Module):
    ...
    def split_for_multi_head(self, tensor: torch.Tensor):
        assert len(
            tensor.shape) == 3, f"Tensor should be 3-dimensional, but the size is {tensor.shape}"
        batch, seq_len, d_model = tensor.size()
        d_tensor = d_model // self.n_head
        tensor = tensor.view(batch, seq_len, self.n_head,
...
class TransformerEncoderLayer(nn.Module):
    ...
    def __init__(self, d_model=64, n_head=8, ffn_hidden=2048, drop_prob=0.1):
        ...
        self.ffn = FFN(d_model=d_model, ffn_hidden=ffn_hidden,
                       drop_prob=drop_prob)
...
if __name__ == "__main__":
    ...
    model = RawformerClassifier(
        C=32, n_encoder=3, transformer_hidden=80).to("cuda:0")
    # x = torch.ones((1, 2, 2, 2), device="cuda:0")
    # print(x)
    # x = model(x)
    # print(x)
    summary(model, (2, 23*29, 32))
