    ...
    def __init__(self, in_channels: int, out_channels: int, is_first_block: bool = False):
        ...
        self.layers = nn.Sequential(
            nn.Conv2d(in_channels=in_channels, out_channels=out_channels,
                      kernel_size=(2, 5), padding=(1, 2), stride=1),
        ...
        if in_channels != out_channels:
            self.downsampler = nn.Sequential(
                nn.Conv2d(in_channels=in_channels, out_channels=out_channels, padding=(
                    0, 1), kernel_size=(1, 3), stride=1)
            )
...
class Conv2DBlock_SE(nn.Module):
    ...
    def __init__(self, in_channels: int, out_channels: int, scale: int = 8, channel_reduction: int = 8):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(in_channels=in_channels, out_channels=self.hidden_channels,
        ...
        for i in range(2, scale+1):
            self.conv2.append(nn.Sequential(
                nn.Conv2d(in_channels=self.sub_channels, out_channels=self.sub_channels, kernel_size=(
                    3, 9), padding=(1, 4)),
...
class Frontend_S(nn.Module):
    ...
    def __init__(self, sinc_kernel_size=128, sample_rate=16000):
        ...
        self.sinc_layer = SincConv(
            in_channels=1, out_channels=70, kernel_size=sinc_kernel_size, sample_rate=sample_rate)
...
class Frontend_SE_W2V(nn.Module):
    """_summary_
    This is frontend of SE-Rawformer
    """
    ...
    def __init__(self, device, cp_path, out_dim):
        ...
        self.ssl_model = SSLModel(device, cp_path, out_dim)
