...
class PositionalAggregator1D(nn.Module):
    ...
    def __init__(self, max_C: int, max_ft: int, device):
        ...
        _2i = torch.arange(
            0, max_C, step=2, device=device).float().unsqueeze(0)

        self.encoding = torch.zeros(
            max_ft, max_C, device=device, requires_grad=False)
...
if __name__ == "__main__":
    ...
    model = PositionalAggregator1D(32, 42*16, "cuda:0").to("cuda:0")
    # x = torch.ones((1, 2, 2, 2), device="cuda:0")
    # print(x)
    # x = model(x)
    # print(x)
    summary(model, (2, 32, 42, 1))
